"""Flow 6 planner: deterministic graph execution + LLM route selection.

This is the route-planning agent from the ``llm-routing`` branch, moved into
``flow_6`` so it no longer conflicts with flow_1.

Unlike flows 1-5, this agent is consumed by ``llm_route_agent_node`` and
exposes ``plan(...)`` rather than the ``initialize/step`` tool-loop surface.
"""
...
class RoutePlanningAgent:
    ...
    def __init__(self, provider: str | None = None, model_name: str | None = None):
        if provider is None or model_name is None:
            resolved_provider, resolved_model = resolve_llm_config("6")
    ...
    def _ensure_run_dir(self) -> Path:
        if self._run_dir is None:
            self._run_dir = make_run_dir(flow="6")
