...
#
# Usage:
#   bash src/nav2_llm_demo/scripts/run_llm_nav.sh <MAP_NAME> [--flow 1|2|3|4|5|6|7]
#
# Examples:
...
#   bash src/nav2_llm_demo/scripts/run_llm_nav.sh warehouse --flow 7     # LLM route planner over a topology graph
...
#   LLM_FLOW=1|2|3|4|5|6|7 Which agent flow to use; --flow CLI arg overrides this.
# ──────────────────────────────────────────────────────────────────────────

set -eo pipefail

if [[ $# -lt 1 ]]; then
  echo "Usage: $0 <MAP_NAME> [--flow 1|2|3|4|5|6|7]" >&2
  exit 1
fi
...
case "${LLM_FLOW}" in
  1|2|3|4|5|6|7) ;;
  *)
    echo "ERROR: --flow must be 1, 2, 3, 4, 5, 6, or 7 (got '${LLM_FLOW}')." >&2
    exit 1
    ;;
esac
export LLM_FLOW
...
# Pre-flight: confirm the agent executable for the chosen flow was actually
# installed by colcon. Flow 7 launches ``llm_route_agent_node`` (the
# topology-graph route planner + hand-rolled executor); all other flows
# launch ``llm_agent_node``. Without this check we'd otherwise see Gazebo
# come up while the agent silently never starts.
if [[ "${LLM_FLOW}" == "7" ]]; then
  LLM_AGENT_EXE="${WORKSPACE_ROOT}/install/nav2_llm_demo/lib/nav2_llm_demo/llm_route_agent_node"
  EXE_NAME="llm_route_agent_node"
else
  LLM_AGENT_EXE="${WORKSPACE_ROOT}/install/nav2_llm_demo/lib/nav2_llm_demo/llm_agent_node"
  EXE_NAME="llm_agent_node"
fi
if [[ ! -x "$LLM_AGENT_EXE" ]]; then
  echo "ERROR: ${EXE_NAME} executable not found at $LLM_AGENT_EXE" >&2
...
# Use a per-invocation temp file so we don't collide with a stale, foreign-
# owned ``/tmp/llm_import_err`` left behind by a previous run (which would
# otherwise make this redirect fail with "Permission denied" and silently
# kill the launch in <1 s).
LLM_IMPORT_ERR_FILE="$(mktemp -t llm_import_err.XXXXXX)"
trap 'rm -f "$LLM_IMPORT_ERR_FILE"' EXIT
if ! python3 -c "from nav2_llm_demo.llm import build_agent" 2>"$LLM_IMPORT_ERR_FILE"; then
  echo "ERROR: nav2_llm_demo.llm failed to import (LLM_FLOW=${LLM_FLOW}):" >&2
  cat "$LLM_IMPORT_ERR_FILE" >&2
  echo >&2
